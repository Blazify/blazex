...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    pub(crate) fn var_assign(&mut self, node: Node) -> Result<BasicValueEnum<'ctx>, Error> {
        match node.clone() {
            Node::VarAssignNode {
                name,
                value,
    ...
    pub(crate) fn var_access(&self, node: Node) -> Result<BasicValueEnum<'ctx>, Error> {
        match node.clone() {
    ...
    pub(crate) fn var_reassign(&mut self, node: Node) -> Result<BasicValueEnum<'ctx>, Error> {
        match node.clone() {
